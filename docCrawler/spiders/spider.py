...
class DocSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        for link in LinkExtractor().extract_links(
            response
        ):  # Extract links from all domains
            if (
                urlparse(self.start_urls[0]).netloc in link.url
    ...
    def save_to_json(self, data):
        with open("./data/output_links.json", "w") as json_file:
