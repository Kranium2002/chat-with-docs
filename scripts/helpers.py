...
from docCrawler.spiders import (
    DocSpider,
        ...
        for link in links:
            try:
                task = asyncio.create_task(
                    _fetch_article_summary(semaphore, session, link)
                )
                tasks.append(task)
            except Exception as e:
...
def get_full_text(path="./data/output_links.json"):
...
def data_loader(path="./data/summary_output.json", jq_schema=".chatwithdocs[].text"):
    """
...
def search_similarity(
    ...
    answer = agent.run(
        "Imagine you are a software developer, you have to help the user with their query as best as you can. When you find multiple documents from simsearch use all of them to generate an answer yourself. Write proper descriptions for your answer and give code if necessary. Use the information from the similarity search to help answer the query. If you find any blog or article you can use webcrawl, webcrawl does not support pdf. Use all tools available to make your answer. Query: "
        + query
    )
    return answer
